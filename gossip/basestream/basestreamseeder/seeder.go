...
type BaseSeeder struct {
	...
	pendingSizeReduced   chan struct{}
...
func New(cfg Config, callbacks Callbacks) *BaseSeeder {
	s := &BaseSeeder{
		...
		pendingSizeReduced:     make(chan struct{}, 1),
...
// waitPendingResponsesBelowLimit waits for pendingResponsesSize to drop below
// the configured limit while remaining responsive to peer disconnects and quit.
func (s *BaseSeeder) waitPendingResponsesBelowLimit() {
	for atomic.LoadInt64(&s.pendingResponsesSize) >= int64(s.cfg.MaxPendingResponsesSize) {
		...
		case peerID := <-s.notifyUnregisteredPeer:
			s.handleUnregister(peerID)
		case <-s.pendingSizeReduced:
			// re-check the condition in the outer for loop
		}
	}
}

func (s *BaseSeeder) handleUnregister(peerID string) {
...
func (s *BaseSeeder) signalPendingSizeReduced() {
	select {
	case s.pendingSizeReduced <- struct{}{}:
	default:
	}
}
...
func (s *BaseSeeder) readerLoop() {
	for {
		...
		case op := <-s.notifyReceivedRequest:
			...
			// add session (with global cap)
			session, ok := s.sessions[sessionIDAndPeer{op.request.Session.ID, op.peer.ID}]
			if !ok && len(s.sessions) >= s.cfg.maxGlobalSessions() {
				op.peer.Misbehaviour(errors.New("too many global sessions"))
				continue
			}
			if !ok {
